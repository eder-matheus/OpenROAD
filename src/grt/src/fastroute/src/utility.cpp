...
void FastRouteCore::updateSlacks(float percentage)
{
  ...
  // updateSlacks() never un-marks, so the cumulative total grows monotonically
  // across calls (the 3D maze passes and final layer assignment). Reports the
  // per-call delta and running total; level 2 lists the nets newly marked.
  if (logger_->debugCheck(GRT, "resAware", 1) && !is_incremental_grt_) {
    int total = 0;
